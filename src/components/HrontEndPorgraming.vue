...
//替换
function stringReplace(value:string,opt:string){
  rpc.value=value.replace(opt,"")
  return rpc.value
}
...
//运算符
const o = ref<boolean>(true)//记录是否是上一个opt
const p = ref()
const jisuan = (opt: string) => {
  let r: string[] = []
  r = errBreak(reister.value)//等式拆分成数组，赋值给r
  //如果是第一次运行就添加空数组
  r.push("")//添加数组
  ...
  if (r[2] != "" && numberLog.value == false) {
    ...
    reister.value += opt
    log = true
  } else if (r[1] == "") {//如果没有操作符就添加操作符,然后记录opt，否则更改操作符
    inputValue2.value += inputValue.value + opt
    numberLog.value = false
    reister.value += opt
  }else{
    inputValue.value=stringReplace(inputValue.value,opt)
    reister.value=stringReplace(reister.value,opt)
  }
  // else if (r[1] != opt || r[1] == opt) {//点击的操作符相同或不同，就删除上一个操作符，重新添加操作符
  //   //问题：如果opt
  //   inputValue2.value = inputValue2.value.replace(p.value, "")//replace，opt替换为空字符
  //   console.log(p.value,inputValue2.value)
  //   inputValue2.value += opt
  //   reister.value = reister.value.replace(p.value, "")
  //   reister.value += opt
